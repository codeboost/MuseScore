...
void LayerManager::addTagClicked()
      ...
      bool ok;
      QString item = QInputDialog::getItem(this, tr("Virtual Guitar: select layer tag"), tr("layer tag"),
         items, 0, false, &ok);
      if (ok && !item.isEmpty()) {
//            uint tagBits = 0;
            for (int i = 0; i < 31; ++i) {
...
void LayerManager::deleteTagClicked()
      ...
      bool ok;
      QString tag = QInputDialog::getItem(this, tr("Virtual Guitar: select layer tag"), tr("layer tag"),
         items, 0, false, &ok);
      if (ok && !tag.isEmpty()) {
            items.removeOne(tag);
